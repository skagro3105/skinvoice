...
async function loadHistoryTable() {
    ...
    const sortInvoicesDesc = (list) => {
        return list.sort((a, b) => {
            const numA = parseInt(String(a.invoiceNumber || a.InvoiceNumber || 0).replace(/[^0-9]/g, ''), 10) || 0;
            const numB = parseInt(String(b.invoiceNumber || b.InvoiceNumber || 0).replace(/[^0-9]/g, ''), 10) || 0;
            return numB - numA;
        });
    };
    ...
    try {
        ...
        // Remove duplicates by uniqueId
        const seen = new Set();
        const filtered = (freshInvoices || []).filter(inv => {
            if (!inv.uniqueId || seen.has(inv.uniqueId)) return false;
            seen.add(inv.uniqueId);
            return true;
        });
        allInvoices = sortInvoicesDesc(filtered);
        console.log('Sorted invoices in JS:', allInvoices.map(i => i.invoiceNumber));
