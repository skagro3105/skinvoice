...
const MASTER_API_URL = typeof APPS_SCRIPT_URL !== 'undefined' ? APPS_SCRIPT_URL : 'https://script.google.com/macros/s/AKfycbyT7tipMrOjdXBxrqnXF4rRVsELlFqhSfzUhCLVtvlKg0wqtCtpAosKPvwkwuRYFuBR/exec';
...
async function postMasterData(action, data) {
  try {
    ...
    if (action === 'addProduct') {
      ...
      res = await api.updateProduct(data);
    } else if (action === 'deleteProduct') {
      res = await api.deleteProduct(data);
    } else if (action === 'addClient') {
      res = await api.addClient(data);
    } else if (action === 'updateClient') {
      res = await api.updateClient(data);
    } else if (action === 'deleteClient') {
      res = await api.deleteClient(data);
    }
    if (!res || !res.success) throw new Error(res?.error || 'Request failed');
    return true;
  } catch (err) {
...
function renderTables() {
  ...
  if (productsData.length === 0) {
    ...
    ptbody.innerHTML = productsData.map(p => `
      <tr>
        <td data-label="ID" style="color:var(--text-muted);font-size:12px;"><span class="table-cell-text single-line mono">${escapeHTML(p.ProductID || '-')}</span></td>
  ...
  if (clientsData.length === 0) {
    ...
    ctbody.innerHTML = clientsData.map(c => {
      ...
      return `
      <tr>
        <td data-label="ID" style="color:var(--text-muted);font-size:12px;"><span class="table-cell-text single-line mono">${escapeHTML(c.ClientID || '-')}</span></td>
