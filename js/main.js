/* =============================================================================
   main.js — SK Agro Chemicals Invoice System
...
async function downloadPDF() {
  ...
  try {
    ...
    for (let i = 0; i < cuts.length - 1; i++) {
      ...
      const img = sc.toDataURL('image/jpeg', 0.97);
    ...
    document.getElementById('pdf-mode-style')?.remove();
    document.getElementById('pdf-clone-wrap')?.remove();
...
async function bootApp(loggedInMobile) {
  ...
  // Start fresh on every reload so uploaded images / invoice draft data do not persist.
  safeStorage.removeItem(LS_KEY);
  if (!window.currentInvoiceId) {
    ['s-client-name', 's-client-addr', 's-client-phone', 's-client-gstin', 's-client-due', 's-due-date'].forEach(id => {
      const el = $(id);
